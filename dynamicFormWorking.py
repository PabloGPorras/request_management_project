...
class DependencyHandler:
    ...
    def setup_dependencies(self, fields, columns, is_json=False):
        for column in columns:
            ...
            field_name_key = "json_column_name" if is_json else "table_name"
            ...
            field_name = column["json_column_name"] if is_json else column["name"]["table_name"]
    ...
    def create_field(self, field_type, column):
        ...
        if isinstance(column, dict):
            ...
            # Handle string fields with automatic max_length validation
            elif field_type == "string":
                field = QtWidgets.QLineEdit()
                if 'max_length' in column.get('type', {}):
                    field.setMaxLength(column['type']['max_length'])
...
class DynamicForm(QtWidgets.QWidget, DependencyHandler):
    ...
    def initUI(self):
        ...
        for column in self.column_definitions:
            if self.should_display_field(column):
                field_name = column["name"]["table_name"]
                ...
                self.layout.addRow(column["name"]["form_name"], self.fields[field_name])
    ...
    def create_json_field(self, column):
        json_group_name = column["name"]["table_name"]
